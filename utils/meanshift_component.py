...
from utils.meanshift_dialog import MeanShift_Dialog
from PyQt5.QtCore import pyqtSignal
from PyQt5.QtWidgets import QDialog,QFileDialog,QMessageBox
from PyQt5.QtGui import QColor
from sklearn.cluster import mean_shift,estimate_bandwidth

class MeanShift_Component(QDialog,MeanShift_Dialog):
    add_layer_signal = pyqtSignal(str,list)
    color_list=[[238,157,168],[35,148,177],[181,45,43],[55,60,71],[6,177,152],[249,2,150],[242,43,18],[98,15,53],
                [110,24,225],[87,121,140],[255,69,83],[255,0,0],[0,255,0],[0,0,255],[220,220,220]]

    def __init__(self):
        super(MeanShift_Component, self).__init__()
        self.setupUi(self)
    ...
    def action_execute_button_clicked(self):
        ...
        #执行mean_shift算法
        bandwidth = estimate_bandwidth(input_features, quantile=float(self.bandwidth_estimate.currentText()), n_samples=int(img_rows*img_cols*0.05))
        #print(bandwidth)
        meanshift_result=mean_shift(input_features,bandwidth=bandwidth,bin_seeding=True,max_iter=int(self.iter_num.currentText()))
        ...
        for i in range(len(cluster_centers)):
            layer_legends.append({'name': 'Cluster' + str(i + 1),
                                  'color': QColor(self.color_list[i][0], self.color_list[i][1], self.color_list[i][2])})
